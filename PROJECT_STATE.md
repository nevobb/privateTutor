...
- Research-based UX corrections applied (branch: design/research-based-ux-corrections-final, PR #30): collapsible left sidebar with 48px mini-rail (topic initial pill, localStorage-persisted), visible collapse button, context strip in TutorConversation showing Topic · Mode · Sources.
- Session transcript API boundary implemented (branch: feat/session-transcript-api-boundary): GET + POST /api/sessions/[sessionId]/messages endpoints, message persistence via Firestore emulator, TutorConversation wired to API — browser no longer calls mock tutor directly.
