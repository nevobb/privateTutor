...
import { createWorkspaceFileSummaryPostHandler } from "../../../src/app/api/workspaces/[workspaceId]/files/[fileId]/summary/route";
...
const mockRunSummary = vi.mocked(uploadedFileApiService.runSummaryLifecycleForFile);
...
describe("POST /api/workspaces/[workspaceId]/files/[fileId]/summary", () => {
  it("returns 401 on auth failure", async () => {
    const handler = createWorkspaceFileSummaryPostHandler(failAuth());
    const response = await handler(
      new Request("http://localhost/api/workspaces/ws-1/files/file-1/summary", { method: "POST" }),
      context("ws-1", "file-1")
    );
  ...
  it("returns 400 for missing params", async () => {
    const handler = createWorkspaceFileSummaryPostHandler(okAuth());
    const response = await handler(
      new Request("http://localhost/api/workspaces/ws-1/files/file-1/summary", { method: "POST" }),
      context("", "")
    );
  ...
  it("returns 404 when workspace or file is missing", async () => {
    mockRunSummary.mockResolvedValueOnce({ ok: false, code: "file_not_found" });
  ...
  it("returns 400 for invalid transition", async () => {
    mockRunSummary.mockResolvedValueOnce({ ok: false, code: "invalid_transition" });
  ...
  it("returns 503 when emulator unavailable", async () => {
    mockRunSummary.mockRejectedValueOnce(new Error("down"));
  ...
  it("returns 200 and serialized updated file", async () => {
    const now = new Date();
    mockRunSummary.mockResolvedValueOnce({
      ok: true,
      file: {
        id: "file-1",
        userId: "alice",
    ...
    mockSerialize.mockReturnValueOnce({ id: "file-1", summaryStatus: "ready" } as never);
    ...
    expect(await response.json()).toEqual({ id: "file-1", summaryStatus: "ready" });
    expect(mockRunSummary).toHaveBeenCalledWith(expect.objectContaining({ userId: "alice" }), "ws-1", "file-1");
  });
});
